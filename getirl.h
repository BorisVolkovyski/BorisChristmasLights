...
void getirl() {      
  ...
  if (Protocol) {
    ...
    switch (Command) {
      ...
      case IR_Key_SetMode_1X :                /////////////////////////////////////////////////////////////////////////// Набор номера выбор эффекта десятки кнопка +10
        if (Protocol == 1) {          //отключить повтор
          IR_New_Mode += 10;
...
/**
 * This is used to reset all the Arduino's so that their millis() counters are all in sync.
 * 
 * @return void
 */
void bootme() {
...
void meshwait() {
  ...
  // Here's our notamesh wait upon keypress. Oh god I'm so sorry there's a delay statement here. At least it's only used upon mode change keypress.
  FastLED.delay(meshdelay * 100);                                   
